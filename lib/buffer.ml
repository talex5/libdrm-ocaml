...
let map_pseudofile ?(pos=0L) dev ~kind (width, height) =
  caml_map_pseudofile dev kind pos width height
...
module Dumb = struct
  ...
  let create dev ~bpp (width, height) =
    ...
    match C.Functions.drmModeCreateDumbBuffer dev width height bpp flags handle pitch size with
    | 0, _ ->
      {
        bpp; width; height;
        handle = !@ handle;
  ...
  let get_map_offset dev handle =
    let offset = Ctypes.(allocate_n uint64_t ~count:1) in
    match C.Functions.drmModeMapDumbBuffer dev handle offset with
  ...
  let map dev t kind =
    ...
    let offset = get_map_offset dev t.handle in
    map_pseudofile dev ~pos:offset ~kind (width, t.height)

  let destroy dev handle =
    match C.Functions.drmModeDestroyDumbBuffer dev handle with
    | 0, _ -> ()
